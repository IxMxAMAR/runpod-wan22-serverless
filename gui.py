"""Simple GUI for RunPod WAN 2.2 T2V/I2V Serverless Endpoints."""
import base64
import json
import os
import subprocess
...
def load_config():
    if CONFIG_FILE.exists():
        with open(CONFIG_FILE) as f:
            return json.load(f)
    return {"api_key": "", "t2v_endpoint": "zqrth2kys0sdsi", "i2v_endpoint": ""}
...
class App:
    ...
    def _build_ui(self):
        ...
        api_entry = ttk.Entry(settings, textvariable=self.api_key_var, show="*", width=50)
        api_entry.grid(row=0, column=1, sticky="ew", padx=4, pady=2)
        ...
        self.prompt_text = tk.Text(prompt_frame, height=4, bg="#16213e", fg="#e0e0e0",
